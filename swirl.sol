...
contract Swirl is Ownable {
    ...
    uint256 supply = sharesSupply[sharesSubject];
    require(supply > amount, "Last share cannot be sold");
    uint256 price = getPrice(supply - amount, amount);
    uint256 protocolFee = price * protocolFeePercent / 1 ether;
    require(sharesBalance[sharesSubject][msg.sender] >= amount, "Not enough shares.");

    sharesBalance[sharesSubject][msg.sender] -= amount;
    sharesSupply[sharesSubject] = supply - amount;

    // Emit the Trade event with additional details
    emit Trade(
        msg.sender,
        sharesSubject,
        false,
    ...
    // Additional logs for the website frontend
    ...
    // Send funds to the seller and protocol fee to the destination
